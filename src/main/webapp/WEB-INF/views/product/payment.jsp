...
<script type="text/javascript" src="https://cdn.iamport.kr/js/iamport.payment-1.1.5.js"></script>
 <script>
	...
	function doPay(){
		...
		var shippingInfo = '<input type="hidden" name="recvName" value="' + recvName + '">';
		shippingInfo += '<input type="hidden" name="recvPhone" value="' + recvPhone + '">';
		shippingInfo += '<input type="hidden" name="recvAddr" value="' + recvAddr + '">';
		shippingInfo += '<input type="hidden" name="shipRequest" value="' + shipRequest + '">';
		
		console.log(shippingInfo);
		$('.buyListForm').append(shippingInfo);
		
         IMP.request_pay({
            pg : 'kakaopay',
            pay_method : 'card',
            merchant_uid : 'merchant_' + new Date().getTime(),
            name : '${prodInfo.pname}' + ' 결제',
            amount : totalPrice,
            buyer_email : '${uInfo.email}',
            buyer_name : '${uInfo.username}',
            buyer_tel : '010-9791-9908',
            buyer_addr : address,
...
<div class="container">
	...
	<div class="receiverInfo">
		<span style="position: relative; top: -5px; font-size: 25px; font-weight: bold; align-content: left;">받는사람 정보</span>&emsp;&emsp;&emsp;
	...
	<table class="prodTbl2">		
			<tr>
				<td class="img1">
					<img src='/resources/image/<c:out value="${prodInfo.image}"/>.jpg' style="width: 100px; height: 100px;">
				</td>
				<td class="title1"><span>${prodInfo.pname}</span></td>
				<td class="price1"><fmt:formatNumber value="${prodInfo.price}"/>원</td>
				<td class="purchase-count1"><span>${prodAmount}</span></td>
			</tr>
	</table>
	<br/><br/><br/>
	...
	<!-- 결제 정보 -->
	<div>
		<span style="position: relative; top: -5px; font-size: 25px; font-weight: bold; align-content: left;">결제 정보</span>
	</div>
	<table class="table3">
		<tr>
			<th>총 상품 가격</th>
			<td><fmt:formatNumber value="${prodInfo.price * prodAmount}"/>원</td>
		<tr>
		<tr>
			<th>배송비</th>
			<c:if test="${prodInfo.price * prodAmount >= 20000}">
		...
		<tr>
			...
			<c:if test="${prodInfo.price * prodAmount >= 20000}">
				<td><fmt:formatNumber value="${prodInfo.price * prodAmount}"/>원</td>
				<input type="hidden" class="totalPrice" value="<fmt:formatNumber value="${prodInfo.price * prodAmount}"/>">
			</c:if>
			<c:if test="${prodInfo.price * prodAmount <= 20000}">
				<td><fmt:formatNumber value="${prodInfo.price * prodAmount + 5000}"/>원</td>
				<input type="hidden" class="totalPrice" value="<fmt:formatNumber value="${prodInfo.price * prodAmount + 5000}"/>">
			</c:if>
		</tr>
		<tr>
			<th>결제방법</th>
	...
	<!-- 구매 테이블에 isert할 값 -->
	<form action="/product/paySuccess" method="post" class="buyListForm">
		<input type="hidden" name="userid" value="${uInfo.userid}">
		<input type="hidden" name="pno" value="${prodInfo.pno}">
		<input type="hidden" name="amount" value="${prodAmount}">
	</form>
